...
class Config(object):
    ...
    MEAN_PIXEL_TENSOR = torch.from_numpy(MEAN_PIXEL.astype(np.float32)).to(device)
    ...
    def __init__(self, options):
        ...
        with torch.no_grad():
            self.URANGE_UNIT = ((torch.arange(self.IMAGE_MAX_DIM, requires_grad=False).to(device).float() + 0.5) / self.IMAGE_MAX_DIM).view((1, -1)).repeat(self.IMAGE_MIN_DIM, 1)
            self.VRANGE_UNIT = ((torch.arange(self.IMAGE_MIN_DIM, requires_grad=False).to(device).float() + 0.5) / self.IMAGE_MIN_DIM).view((-1, 1)).repeat(1, self.IMAGE_MAX_DIM)
            self.ONES = torch.ones(self.URANGE_UNIT.shape, requires_grad=False).to(device)
            pass
        ...
        self.PREDICT_DEPTH = False # 007 - True
    ...
    def loadAnchorPlanes(self, anchor_type = ''):
        ...
        with torch.no_grad():
            if self.ANCHOR_TYPE == 'none':
                ...
                self.ANCHOR_PLANES_TENSOR = torch.from_numpy(self.ANCHOR_PLANES.astype(np.float32)).to(device)
                ...
                self.NUM_PARAMETERS = 4
                self.ANCHOR_NORMALS_TENSOR = torch.from_numpy(self.ANCHOR_NORMALS.astype(np.float32)).to(device)
                self.ANCHOR_OFFSETS_TENSOR = torch.from_numpy(self.ANCHOR_OFFSETS.astype(np.float32)).to(device)
