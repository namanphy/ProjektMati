...
from model.bboxDecoder.utils.utils import xyxy2xywh, xywh2xyxy
from model.bboxDecoder.utils.datasets import *
...
class TriclopsDataset(Dataset):
    ...
    def __init__(self, bbox_params):
        ...
        cache_images = bbox_params.get('cache_images', True)
        single_cls = bbox_params.get('single_cls', False)
        ...
        self.img_size = bbox_params.get('img_size', 416)
        self.augment = bbox_params.get('augment', False)
        self.hyp = bbox_params.get('hyp', None)
        self.image_weights = bbox_params.get('image_weights', False)
        ...
        if self.rect:
            ...
            # Sort by aspect ratio of images
            ...
            # Bach wise image shapes
            self.batch_shapes = np.ceil(np.array(shapes) * self.img_size / 64.).astype(np.int) * 64

        # Preload labels (required for weighted CE training)
        self.imgs = [None] * n
        self.labels = [None] * n
        if cache_labels or self.image_weights:  # cache labels for faster training
